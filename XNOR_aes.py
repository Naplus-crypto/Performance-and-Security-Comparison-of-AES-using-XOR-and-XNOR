...
def add_round_key_xnor(s, k):
    for i in range(4):
        for j in range(4):
            s[i][j] = ~(s[i][j] ^ k[i][j]) & 0xFF
...
def mix_single_column_xnor(a):
    # ใช้ XNOR แทนการคำนวณแบบ Rijndael ดั้งเดิม
    t = a[0] ^ a[1] ^ a[2] ^ a[3]
    u = a[0]
    a[0] = ~(a[0] ^ t) & 0xFF ^ xtime(a[0] ^ a[1])
    a[1] = ~(a[1] ^ t) & 0xFF ^ xtime(a[1] ^ a[2])
    a[2] = ~(a[2] ^ t) & 0xFF ^ xtime(a[2] ^ a[3])
    a[3] = ~(a[3] ^ t) & 0xFF ^ xtime(a[3] ^ u)

def mix_columns_xnor(s):
    for i in range(4):
        mix_single_column_xnor(s[i])

def inv_mix_single_column_xnor(a):
    # ใช้การคำนวณย้อนกลับที่ถูกต้องของ mix_single_column_xnor
...
def inv_mix_columns_xnor(s):
    for i in range(4):
        inv_mix_single_column_xnor(s[i])
...
def xnor_bytes(a, b):
    """
    Perform XNOR between two byte arrays and return the result as a new byte array.
    """
    # ดำเนินการ XNOR และกลับบิตของผลลัพธ์
    return bytes(~(i ^ j) & 0xFF for i, j in zip(a, b)) 
    # & 0xFF เพื่อรักษาผลลัพธ์ให้อยู่ในขอบเขตของ 1 ไบต์

def inc_bytes(a):
    """ Returns a new byte array with the value increment by 1 """
    out = list(a)  # แปลง byte array a ให้เป็น list ของ integers
    for i in reversed(range(len(out))):  # ทำการ loop ย้อนกลับจากบิตขวาสุดไปทางซ้าย
        if out[i] == 0xFF:  # ถ้าค่าปัจจุบันเป็น 0xFF (255 ในฐาน 10)
            out[i] = 0  # ตั้งค่าเป็น 0 และ carry ไปยังบิตถัดไปทางซ้าย
        else:
            out[i] += 1  # ถ้าค่าปัจจุบันไม่ใช่ 0xFF ให้เพิ่มค่า 1
            break  # เมื่อเพิ่มค่าเสร็จแล้วให้หยุด loop
    return bytes(out)  # แปลง list กลับเป็น byte array แล้ว return
...
def unpad(plaintext):
    ...
    assert padding_len > 0 and padding_len <= 16, "Invalid padding length"
    message, padding = plaintext[:-padding_len], plaintext[-padding_len:]
    # Verify padding
    assert all(p == padding_len for p in padding), "Invalid padding content"
    return message
...
class AES:
    ...
    def _expand_key(self, master_key):
        ...
        while len(key_columns) < (self.n_rounds + 1) * 4:
            ...
            if len(key_columns) % iteration_size == 0:
                ...
                # XNOR with first byte of R-CON, since the others bytes of R-CON are 0.
                word[0] = ~(word[0] ^ r_con[i]) & 0xFF
                i += 1
            ...
            # XNOR with equivalent word from previous iteration.
            word = xnor_bytes(word, key_columns[-iteration_size])
    ...
    def display_key_info(self):
        """
        Displays the size of the key and the number of rounds.
        """
        key_size = len(self.master_key)
        print(f"Key size: {key_size} bytes")
        print(f"Number of rounds: {self.n_rounds}")
    
    def display_padding_info(self, plaintext):
        """
        This function checks and displays the padding and its size.
        """
        padding_len = plaintext[-1]
    ...
    def encrypt_block(self, plaintext):
        ...
        add_round_key_xnor(plain_state, self._key_matrices[0])

        for i in range(1, self.n_rounds):
            sub_bytes(plain_state)
            shift_rows(plain_state)
            mix_columns_xnor(plain_state)
            add_round_key_xnor(plain_state, self._key_matrices[i])

        sub_bytes(plain_state)
        shift_rows(plain_state)
        add_round_key_xnor(plain_state, self._key_matrices[-1])
    ...
    def decrypt_block(self, ciphertext):
        ...
        add_round_key_xnor(cipher_state, self._key_matrices[-1])
        inv_shift_rows(cipher_state)
        inv_sub_bytes(cipher_state)

        for i in range(self.n_rounds - 1, 0, -1):
            add_round_key_xnor(cipher_state, self._key_matrices[i])
            inv_mix_columns_xnor(cipher_state)
            inv_shift_rows(cipher_state)
            inv_sub_bytes(cipher_state)

        add_round_key_xnor(cipher_state, self._key_matrices[0])
    ...
    def decrypt_ecb(self, ciphertext):
        """
        Decrypts the ciphertext using ECB mode and PKCS#7 padding.
        """
        blocks = []
